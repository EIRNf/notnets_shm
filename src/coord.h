...
class coord {
    ...
    public:
        ...
        int service_requests(std::function<key_pair(int)> allocation, int key_seed); //Continue servicing requests, pass function as desired work to be done. 
...
int coord::service_requests(std::function<key_pair(int)> allocation, int key_seed){
    coord_header *header = this->get_coord_header(shm->shmaddr);
