...
const char* mu_name = "/coord-mutex";
...
class coord {
    ...
    private:
        ...
        std::binary_semaphore *mu; //use shared_mutext for now, might be replaced by another
...
int coord::create(shared_memory_region *shm){
    ...
    this->mu = &shared_mutex_init((char*)mu_name);
    if (this->mu->ptr == NULL) {
        perror("mutex error");
        exit(1);
    }
...
key_pair coord::try_request_keys(int client_id){
    ...
    pthread_mutex_lock(this->mu->ptr);
    for(int i = 0; i <available_coord_slots; i++){
    ...
    pthread_mutex_unlock(this->mu->ptr);
